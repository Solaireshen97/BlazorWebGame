...
public class GameStateService : IAsyncDisposable
{
    ...
    public void UseItem(string itemId) { if (Player == null) return; var item = ItemData.GetItemById(itemId); if (item is not Consumable consumable) return; RemoveItemFromInventory(itemId, 1, out int removedCount); if (removedCount <= 0) return; switch (consumable.Effect) { case ConsumableEffectType.Heal: Player.Health = Math.Min(Player.GetTotalMaxHealth(), Player.Health + (int)consumable.EffectValue); break; case ConsumableEffectType.StatBuff: if (consumable.BuffType.HasValue && consumable.DurationSeconds.HasValue) { var existingBuff = Player.ActiveBuffs.FirstOrDefault(b => b.BuffType == consumable.BuffType.Value); if (existingBuff != null) { existingBuff.TimeRemainingSeconds = consumable.DurationSeconds.Value; } else { Player.ActiveBuffs.Add(new Buff { SourceItemId = consumable.Id, BuffType = consumable.BuffType.Value, BuffValue = (int)consumable.EffectValue, TimeRemainingSeconds = consumable.DurationSeconds.Value }); } } break; } NotifyStateChanged(); }

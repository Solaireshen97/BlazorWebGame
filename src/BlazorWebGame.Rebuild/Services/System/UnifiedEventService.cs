...
namespace BlazorWebGame.Rebuild.Services.System
{
    /// <summary>
    /// 统一事件服务 - 服务端实现
    /// 整合新的统一事件队列系统与现有架构
    ...
    public class LegacyCompatibilityHandler : IUnifiedEventHandler
    {
        ...
        public async Task HandleAsync(UnifiedEvent evt)
        {
            ...
            {
                var legacyArgs = new GameEventArgs(
        ...
        private GameEventType? ConvertToLegacyEventType(ushort newEventType)
        {
            return newEventType switch
            {
                GameEventTypes.BATTLE_STARTED => GameEventType.CombatStarted,
                GameEventTypes.BATTLE_ENDED => GameEventType.CombatEnded,
                GameEventTypes.CHARACTER_LEVEL_UP => GameEventType.CharacterLevelUp,
                GameEventTypes.EXPERIENCE_GAINED => GameEventType.ExperienceGained,
                GameEventTypes.ITEM_ACQUIRED => GameEventType.ItemAcquired,
                GameEventTypes.GOLD_CHANGED => GameEventType.GoldChanged,
                GameEventTypes.STATE_CHANGED => GameEventType.GenericStateChanged,
                _ => null
            };
        }
    }

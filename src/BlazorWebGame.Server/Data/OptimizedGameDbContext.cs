...
/// <summary>
/// 优化的游戏数据库上下文 - 包含性能优化配置
/// </summary>
public class OptimizedGameDbContext : DbContext
{
    private readonly ILogger<OptimizedGameDbContext>? _logger;

    public OptimizedGameDbContext(DbContextOptions<OptimizedGameDbContext> options) : base(options)
    {
    }

    public OptimizedGameDbContext(DbContextOptions<OptimizedGameDbContext> options, ILogger<OptimizedGameDbContext> logger) 
        : base(options)
    {
        _logger = logger;
    }
    ...
    private void CreatePerformanceIndexes(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<BattleRecordEntity>()
            .HasIndex(e => new { e.StartedAt, e.EndedAt, e.Status })
            .HasDatabaseName("IX_BattleRecords_TimeRange");
            
        // 分页查询优化
        modelBuilder.Entity<ActionTargetEntity>()
            .HasIndex(e => new { e.PlayerId, e.CreatedAt, e.IsCompleted })
            .HasDatabaseName("IX_ActionTargets_PlayerPaging");
    }

...
/// <summary>
/// SQLite数据存储服务实现 - 基于Entity Framework Core，使用DbContextFactory
/// </summary>
public class SqliteDataStorageService : IDataStorageService
{
    ...
    public async Task<ApiResponse<bool>> DeletePlayerAsync(string playerId)
    {
        try
        {
            ...
            if (player != null)
            {
                ...
                // 清理相关数据
                var actionTargets = await _context.ActionTargets.Where(at => at.PlayerId == playerId).ToListAsync();
                _context.ActionTargets.RemoveRange(actionTargets);
                
                var offlineData = await _context.OfflineData.Where(od => od.PlayerId == playerId).ToListAsync();
    ...
    public async Task<TeamStorageDto?> GetTeamByCaptainAsync(string captainId)
    {
        try
        {
            var team = await _context.Teams.FirstOrDefaultAsync(t => t.CaptainId == captainId);
            return team != null ? MapToDto(team) : null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<TeamStorageDto?> GetTeamByPlayerAsync(string playerId)
    {
        try
        {
            var team = await _context.Teams
                .FirstOrDefaultAsync(t => t.MemberIdsJson.Contains($"\"{playerId}\""));
            return team != null ? MapToDto(team) : null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionTargetStorageDto?> GetCurrentActionTargetAsync(string playerId)
    {
        ...
        {
            _logger.LogError(ex, "Failed to get current action target for player ID: {SafePlayerId}", SafeLogId(playerId));
            return null;
        }
    }
    ...
    public async Task<ApiResponse<ActionTargetStorageDto>> SaveActionTargetAsync(ActionTargetStorageDto actionTarget)
    {
        try
        {
            ...
            var existing = await _context.ActionTargets.FindAsync(actionTarget.Id);
            if (existing != null)
            {
                _context.Entry(existing).CurrentValues.SetValues(entity);
            }
            ...
            {
                await _context.ActionTargets.AddAsync(entity);
            }
    ...
    public async Task<ApiResponse<BattleRecordStorageDto>> SaveBattleRecordAsync(BattleRecordStorageDto battleRecord)
    {
        try
        {
            ...
            var existing = await _context.BattleRecords.FindAsync(battleRecord.Id);
            if (existing != null)
            {
                _context.Entry(existing).CurrentValues.SetValues(entity);
            }
            ...
            {
                await _context.BattleRecords.AddAsync(entity);
            }
    ...
    public async Task<ApiResponse<OfflineDataStorageDto>> SaveOfflineDataAsync(OfflineDataStorageDto offlineData)
    {
        try
        {
            ...
            var existing = await _context.OfflineData.FindAsync(offlineData.Id);
            if (existing != null)
            {
                _context.Entry(existing).CurrentValues.SetValues(entity);
            }
            ...
            {
                await _context.OfflineData.AddAsync(entity);
            }
    ...
    public async Task<ApiResponse<bool>> MarkOfflineDataSyncedAsync(List<string> offlineDataIds)
    {
        try
        {
            ...
            var offlineDataToUpdate = await _context.OfflineData
                .Where(od => offlineDataIds.Contains(od.Id))
                .ToListAsync();
            
            foreach (var offlineData in offlineDataToUpdate)
            {
            ...
            return new ApiResponse<bool>
            {
                Success = true,
                Data = true,
                Message = $"已标记 {offlineDataToUpdate.Count} 条离线数据为已同步"
            };
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse<int>> CleanupSyncedOfflineDataAsync(DateTime olderThan)
    {
        try
        {
            ...
            _logger.LogInformation("Cleaned up {RemovedCount} synced offline data records older than the specified date", toRemove.Count);
            
            return new ApiResponse<int>
            {
                Success = true,
                Data = toRemove.Count,
                Message = $"清理了 {toRemove.Count} 条已同步的旧离线数据"
            };
        }
        catch (Exception ex)
        {

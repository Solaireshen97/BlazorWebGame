...
public class DataStorageIntegrationService
{
    ...
    /// 从现有Player模型同步到数据存储
    /// </summary>
    public async Task<bool> SyncPlayerToStorageAsync(BlazorWebGame.Models.Player player)
    {
        try
        {
    ...
    /// </summary>
    public async Task<BlazorWebGame.Models.Player?> LoadPlayerFromStorageAsync(string playerId)
    {
        try
        {
    ...
    /// 从现有Party模型同步到数据存储
    /// </summary>
    public async Task<bool> SyncPartyToStorageAsync(BlazorWebGame.Models.Party party)
    {
        try
        {
    ...
    /// 从数据存储加载Party数据到现有模型
    /// </summary>
    public async Task<BlazorWebGame.Models.Party?> LoadPartyFromStorageAsync(string teamId)
    {
        try
        {
    ...
    private PlayerStorageDto ConvertPlayerToStorageDto(BlazorWebGame.Models.Player player)
    {
    ...
    private BlazorWebGame.Models.Player ConvertStorageDtoToPlayer(PlayerStorageDto dto)
    {
        var player = new BlazorWebGame.Models.Player
        {
            Id = dto.Id,
            Name = dto.Name,
            Health = dto.Health,
    ...
    private TeamStorageDto ConvertPartyToTeamDto(BlazorWebGame.Models.Party party)
    {
        return new TeamStorageDto
        {
            ...
            MaxMembers = BlazorWebGame.Models.Party.MaxMembers,
            Status = "Active",
    ...
    private BlazorWebGame.Models.Party ConvertTeamDtoToParty(TeamStorageDto dto)
    {
        var party = new BlazorWebGame.Models.Party
        {
    ...
    /// 批量同步多个玩家数据到存储
    /// </summary>
    public async Task<int> BatchSyncPlayersToStorageAsync(IEnumerable<BlazorWebGame.Models.Player> players)
    {
